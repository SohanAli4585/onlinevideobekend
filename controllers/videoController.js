...
const uploadVideo = (req, res) => {
    ...
    if (!title || !user_id || !video_url) return res.status(400).json({ message: "সব ফিল্ড পূরণ করুন" });
...
const likeVideo = (req, res) => {
    ...
    db.query("INSERT INTO likes (video_id, user_id) VALUES (?, ?)", [video_id, user_id], (err, result) => {
        if (err) return res.status(500).json(err);
...
const commentVideo = (req, res) => {
    ...
    db.query("INSERT INTO comments (video_id, user_id, comment) VALUES (?, ?, ?)", [video_id, user_id, comment], (err, result) => {
        if (err) return res.status(500).json(err);
...
const shareVideo = (req, res) => {
    const video_id = req.params.id;
    const { user_id } = req.body;
    db.query("INSERT INTO shares (video_id, user_id) VALUES (?, ?)", [video_id, user_id], (err, result) => {
        if (err) return res.status(500).json(err);
